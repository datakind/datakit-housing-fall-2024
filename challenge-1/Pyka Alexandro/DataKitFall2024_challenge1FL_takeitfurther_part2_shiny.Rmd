...
Progressing from the first [take it further analysis](https://github.com/PykaAlexandro/DataKitFall2024/blob/main/DataKitFall2024_challenge1FL_takeitfurther_part1_shiny.Rmd), we welcome Chris Chenet's suggestions on how to advance it, picking up the Unit Size one among them, which is formulated as such:
...
selectInput("county", 
            label = "County",
            choices = factor(sort(unique(data_1_FL_takeitfurther_part2$County))), 
...
selectInput("county", 
            ...
            choices = factor(sort(unique(data_1_FL_takeitfurther_part2$County))), 
            selected = "Alachua County")
renderTable({data_1_FL_takeitfurther_part2 %>%
#we pick the county of choice
    filter(County == input$county) %>%
#we create the variables for max 2 people / more than 2 people
    mutate("Max 2 people (Owner)" = rowSums(pick(`1-person (owner)`:`2-person (owner)`)),
...
and a dynamic table (downloadable as an excel or pdf file) to explore specific census tracts.
...
selectInput("county", 
            ...
            choices = factor(sort(unique(data_1_FL_takeitfurther_part2$County))), 
            selected = "Alachua County")
renderDT({data_1_FL_takeitfurther_part2 %>%
#we pick the county of choice
    filter(County == input$county) %>%
...
The following table complements the graph with hard numbers:
...
Lastly we will examine the distribution of gross rents by the number of bedrooms,
...
We can more precisely quantify the different behaviours in the following table,
...
selectInput("county", 
            ...
            choices = factor(sort(unique(data_1_FL_takeitfurther_part2$County))), 
            selected = "Alachua County")
renderPlot({data_1_FL_takeitfurther_part2 %>%
#we pick the county of choice
    filter(County == input$county) %>%
#we select the variables of interest
    select(1, 17:28) %>%
...
#we plot the data
    ...
    labs(x = NULL,
         y = NULL,
         title = "Breakdown of Unit Size by Tenure")})
renderTable({data_1_FL_takeitfurther_part2 %>%
#we pick the county of choice
    filter(County == input$county) %>%
